...
public class PenrollDao extends AbstractReportDao {
	...
	public SqlRowSet query(Object[] parameter) {
		...
		sql.append("SELECT S4.APPROVE_EMBOSSNAME2 AS MEMBER_ID, ");
		sql.append("       (SELECT PREFIXNAME_ENAME ");
		sql.append("        FROM   NFE_MS_PREFIXNAME  ");
		sql.append("        WHERE  PREFIXNAME_ID = S1.APPSUP_PREFIXNAME) AS TITLE_NAME,  ");
		sql.append("       S1.APPSUP_ENGFNAME AS FIRST_NAME, ");
		sql.append("       S1.APPSUP_ENGLNAME AS LAST_NAME, ");
		sql.append("       'H' AS ADDRESS_INDICATOR, ");
		sql.append("       S1.APPSUP_CURRENTADDR_LINE1 AS HOME_ADDRESS_LINE1, ");
		sql.append("       S1.APPSUP_CURRENTADDR_LINE2 AS HOME_ADDRESS_LINE2, ");
		sql.append("       (SELECT D.DISTRICT_TNAME ");
		sql.append("        FROM   NFE_MS_DISTRICT D ");
		sql.append("        WHERE  D.DISTRICT_ID = S1.APPSUP_CURRENTADDR_DISTRICT) AS HOME_ADDRESS_LINE3, ");
		sql.append("       ''  AS HOME_ADDRESS_LINE4, ");
		sql.append("       (SELECT S.SUBPROVINCE_TNAME ");
		sql.append("        FROM   NFE_MS_SUBPROVINCE S ");
		sql.append("        WHERE  S.SUBPROVINCE_ID = S1.APPSUP_CURRENTADDR_SUBPROVINCE) AS HOME_CITY_NAME, ");
		sql.append("       (SELECT P.PROVINCE_TNAME ");
		sql.append("        FROM   NFE_MS_PROVINCE P ");
		sql.append("        WHERE  P.PROVINCE_ID = S1.APPSUP_CURRENTADDR_PROVINCE) AS HOME_PROVINCE_NAME, ");
		sql.append("       'TH' AS HOME_COUNTRY, ");
		sql.append("       S1.APPSUP_CURRENTADDR_ZIPCODE AS HOME_POSTAL_CODE, ");
		sql.append("       '' AS COMPANY_NAME, ");
		sql.append("       '' AS BU_ADDRESS_LINE1, ");
		sql.append("       '' AS BU_ADDRESS_LINE2, ");
		sql.append("       '' AS BU_ADDRESS_LINE3, ");
		sql.append("       '' AS BU_ADDRESS_LINE4,  ");
		sql.append("       '' AS BU_CITY_NAME,  ");
		sql.append("       '' AS BU_PROVINCE_NAME, ");
		sql.append("       '' AS BU_COUNTRY, ");
		sql.append("       '' AS BU_POSTAL_CODE, ");
		sql.append("       'H' AS TELEPHONE_INDICATOR, ");
		sql.append("       (SELECT P.PROVINCE_PHONECODE ");
		sql.append("        FROM   NFE_MS_PROVINCE P ");
		sql.append("        WHERE  P.PROVINCE_ID = S1.APPSUP_CURRENTADDR_PROVINCE) AS HOME_TEL_AREA_CODE, ");
		sql.append("       S1.APPSUP_CURRENTADDR_PHONENO AS HOME_TEL_NO, ");
		sql.append("       S1.APPSUP_CURRENTADDR_PHONENOEXT AS HOME_TEL_EXT, ");
		sql.append("       '' AS BU_TEL_AREA_CODE, ");
		sql.append("       '' AS BU_TEL_NO, ");
		sql.append("       '' AS BU_TEL_EXT, ");
		sql.append("       '66' AS MOBILE_AREA_CODE, ");
		sql.append("       S1.APPSUP_CURRENTADDR_MOBILENO AS MOBILE_NO, ");
		sql.append("       '' AS MOBILE_NO_EXT, ");
		sql.append("       '' AS FAX_AREA_CODE, ");
		sql.append("       '' AS FAX_NO, ");
		sql.append("       '' AS FAX_EXT, ");
		sql.append("       S1.APPSUP_CURRENTADDR_EMAIL AS EMAIL, ");
		sql.append("       TO_CHAR(S1.APPSUP_DOB, 'DDMMYYYY') AS BIRTH_DATE ");
		...
		sql.append("AND    EXISTS (SELECT 'X' ");
		sql.append("               FROM   NFE_MS_SUBPRODUCT ");
		sql.append("               WHERE  SUBPRODUCT_ID = S4.APPROVE_SUBPRODUCTID ");
		sql.append("               AND    SUBPRODUCT_PRODUCTID = S4.APPROVE_PRODUCTID ");
		sql.append("               AND    SUBPRODUCT_CODE IN ('P32', 'T24')) ");
		sql.append("AND    EXISTS (SELECT 'X' ");
		sql.append("               FROM   NFE_APP_STATUSTRACKING ");
		sql.append("               WHERE  STATUSTRACKING_ID > 0 ");
