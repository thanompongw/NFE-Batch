 ...
 *
 */
@Repository(value = "letterApproveCreditLimitLA05Dao")
public class LetterApproveCreditLimitLA05Dao extends AbstractBatchDao {

	/**
	 * 
	 */
	public LetterApproveCreditLimitLA05Dao() {
	...
	public SqlRowSet queryDetail(Object[] parameter) {
		...
		sql.append("SELECT NVL(T.CLAPPLICATION_TNAME || ' ' || T.CLAPPLICATION_TSURNAME, ' ') AS THAI_NAME, ");
		...
		sql.append("        AND   SLADDRESS_ADDRESSTYPE = A.STMMAILINGFLAG) AS ADDRESSLINE1, ");
		sql.append("       (SELECT NVL(SLADDRESS_ADDRESSLINE2, ' ') ");
		sql.append("        FROM NFE_APP_SLADDRESS ");
		sql.append("        WHERE SLADDRESS_ID > 0 ");
		sql.append("        AND   SLADDRESS_APPNO = A.CLAPPLICATION_NO ");
		sql.append("        AND   SLADDRESS_ADDRESSTYPE = A.STMMAILINGFLAG) AS ADDRESSLINE2, ");
		sql.append("       (SELECT NVL(SLADDRESS_ADDRESSLINE3 || ' ' || SLADDRESS_ADDRESSLINE4,' ') ");
		sql.append("        FROM NFE_APP_SLADDRESS ");
		sql.append("        WHERE SLADDRESS_ID > 0 ");
		sql.append("        AND   SLADDRESS_APPNO = A.CLAPPLICATION_NO ");
		sql.append("        AND   SLADDRESS_ADDRESSTYPE = A.STMMAILINGFLAG) AS ADDRESSLINE3, ");
		sql.append("       (SELECT NVL(SLADDRESS_CITY, ' ') ");
		sql.append("        FROM NFE_APP_SLADDRESS ");
		sql.append("        WHERE SLADDRESS_ID > 0 ");
		sql.append("        AND   SLADDRESS_APPNO = A.CLAPPLICATION_NO ");
		sql.append("        AND   SLADDRESS_ADDRESSTYPE = A.STMMAILINGFLAG) AS ADDRESSLINE4, ");
		...
		sql.append("       A.OLDCREDITLIMIT AS OLD_CREDIT_LIMIT, ");
		sql.append("       A.NEWCREDITLIMIT AS NEW_CREDIT_LIMIT, ");
		...
		sql.append("        AND    STATUSTRACKING_APPNO = A.CLAPPLICATION_NO ");
		sql.append("        AND    STATUSTRACKING_STATUS = '8F') AS DATES, ");
		sql.append("       T.CLAPPLICATION_NO AS APPLICATION_NO ");
		sql.append("FROM   NFE_CLAPPLICATION T ");
		sql.append("JOIN  ");
		sql.append("( ");
		sql.append("SELECT T1.CLAPPLICATION_NO, ");
		sql.append("       MAX(T2.CLPRODUCT_STMMAILINGFLAG) AS STMMAILINGFLAG, ");
		sql.append("       MAX(T2.CLPRODUCT_ACCOUNTCREDITLIMIT) AS OLDCREDITLIMIT, ");
		sql.append("       MAX(T4.APPROVE_CREDITLIMIT) AS NEWCREDITLIMIT ");
		...
		sql.append(") A ");
		sql.append("    ON A.CLAPPLICATION_NO = T.CLAPPLICATION_NO ");
