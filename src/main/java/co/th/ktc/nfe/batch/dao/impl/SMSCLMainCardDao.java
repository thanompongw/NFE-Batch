 ...
 *
 */
@Repository(value = "smsCLMainCardDao")
public class SMSCLMainCardDao extends AbstractBatchDao {

	/**
	 * 
	 */
	public SMSCLMainCardDao() {
	...
	public SqlRowSet queryDetail(Object[] parameter) {
		...
		sql.append("SELECT  T.CLAPPLICATION_MOBILEPHONE AS MOBILE_NO, ");
		sql.append("        (CASE ");
		sql.append("                        WHEN LENGTH(T.CLAPPLICATION_ENAME || ' ' || T.CLAPPLICATION_ESURNAME) <= 20 ");
		sql.append("                        THEN NVL(T.CLAPPLICATION_ENAME || ' ' || T.CLAPPLICATION_ESURNAME, ' ') ");
		sql.append("                        ELSE SUBSTR((T.CLAPPLICATION_ENAME || '.' || SUBSTR(T.CLAPPLICATION_ESURNAME, 0, 1)), 0, 20) ");
		sql.append("                END) AS ENG_NAME, ");
		sql.append("           A.CREDIT_LIMIT ");
		sql.append("FROM NFE_CLAPPLICATION T ");
		sql.append("JOIN ( ");
		sql.append("SELECT T1.CLAPPLICATION_NO, ");
		sql.append("       T2.CLPRODUCT_GROUPPRODUCT, ");
		...
		sql.append("         T2.CLPRODUCT_GROUPPRODUCT) A ");
