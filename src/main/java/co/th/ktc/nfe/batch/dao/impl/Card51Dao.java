...
public class Card51Dao extends AbstractBatchDao {
	...
	public SqlRowSet queryDetail(Object[] parameter) {
		...
		sql.append("        RPAD((SELECT PREFIXNAME_ENAME ");
		sql.append("              FROM NFE_MS_PREFIXNAME ");
		sql.append("              WHERE  PREFIXNAME_ID = T1.APP_PREFIXNAME)" );
		sql.append("             || ' '|| NVL(T4.APPROVE_EMBOSSNAME1, ' '), 30, ' ') AS EMBOSSLINE1, ");
		sql.append("        RPAD(NVL(T4.APPROVE_EMBOSSNAME2, ' '), 30, ' ') AS EMBOSSLINE2, ");
		sql.append("        RPAD((CASE  ");
		sql.append("                  WHEN T3.RESOLVE_CREDITTYPE = 'M'  ");
		...
		sql.append("        RPAD(NVL((SELECT PIF_MARRIAGE_NOCHILDREN ");
		sql.append("                  FROM   NFE_APP_PIF ");
		sql.append("                  WHERE  PIF_APPNO = T1.APP_NO), 0), 2, ' ') AS NO_OF_CHILD, ");
		...
		sql.append("        RPAD(NVL(T1.APP_AGENT, ' '), 20, ' ') AS AGEN_CODE, ");
		sql.append("        RPAD(NVL(T1.APP_BRANCH, ' '), 3, ' ') AS BRANCH_CODE, ");
		...
		sql.append("        RPAD(NVL((SELECT NVL(PIF_CURRENTADDR_MOBILENO, ' ') ");
		sql.append("                  FROM   NFE_APP_PIF ");
		sql.append("                  WHERE  PIF_APPNO = T1.APP_NO), ' '), 16, ' ')  AS MOBILE, ");
		sql.append("        RPAD(NVL((SELECT NVL(APPOCCUPATION_PHONENO1, ' ') ");
		sql.append("                  FROM   NFE_APP_OCCUPATION  ");
		sql.append("                  WHERE  APPOCCUPATION_APPNO = T1.APP_NO), ' '), 25, ' ') AS OFFICE_PHONE1, ");
		sql.append("        RPAD(NVL((SELECT NVL(APPOCCUPATION_PHONENO2, ' ') ");
		sql.append("                  FROM   NFE_APP_OCCUPATION  ");
		...
		sql.append("        RPAD(NVL((SELECT NVL(PIF_CURRENTADDR_EMAIL, ' ')  ");
		sql.append("                  FROM   NFE_APP_PIF ");
		...
		sql.append("        RPAD(NVL((SELECT NVL(E.EDUDEGREE_TNAME, ' ') ");
		...
		sql.append("        RPAD(NVL((SELECT NVL(APPOCCUPATION_WORKPLACE, ' ') ");
		sql.append("                  FROM   NFE_APP_OCCUPATION  ");
		...
		sql.append("        '000000000000000000000000000000' AS ANNUAL_INC, ");
		...
		sql.append("        RPAD(NVL(T4.APPROVE_UPDATEBY, ' '), 20, ' ') AS DONE_BY, ");
