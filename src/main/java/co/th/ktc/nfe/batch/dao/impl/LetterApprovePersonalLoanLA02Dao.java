 ...
 *
 */
@Repository(value = "letterApprovePersonalLoanLA02Dao")
public class LetterApprovePersonalLoanLA02Dao extends AbstractBatchDao {

	/**
	 * 
	 */
	public LetterApprovePersonalLoanLA02Dao() {
	...
	public SqlRowSet queryHeader(Object[] parameter) {
		...
		sql.append("            ), '-') AS BANK, ");
		sql.append("       (SELECT NVL(TRANSFERLOAN_ACCNAME, '-') ");
		sql.append("        FROM   NFE_APP_TRANSFERLOAN ");
		sql.append("        WHERE  TRANSFERLOAN_ID > 0  ");
		sql.append("        AND    TRANSFERLOAN_APPNO = T1.APP_NO) AS ACCOUNT_NAME, ");
		sql.append("       (SELECT NVL(TRANSFERLOAN_ACCNO, '-') ");
		sql.append("        FROM   NFE_APP_TRANSFERLOAN ");
		sql.append("        WHERE  TRANSFERLOAN_ID > 0  ");
		sql.append("        AND    TRANSFERLOAN_APPNO = T1.APP_NO) AS ACCOUNT_NO ");
		sql.append("FROM   NFE_APPLICATION T1 ");
		sql.append("       LEFT JOIN NFE_APP_PRODUCT T2  ");
		sql.append("           ON  T2.APPPRODUCT_ID > 0  ");
		sql.append("           AND T2.APPPRODUCT_APPNO = T1.APP_NO ");
		...
		sql.append("WHERE  T1.APP_NO <> ' ' ");
		sql.append("AND    T1.APP_SOURCECODE NOT IN ('WSC', 'WSX', 'BBC') ");
		sql.append("AND    T3.RESOLVE_STATUSCODE = '8A' ");
